...
describe('colour schemes', () => {
  ...
  it('keeps one potion per design system, with stable ids', () => {
    ...
    expect(SCHEMES.map(s => s.id)).toEqual(['greek', 'foudre', 'slush', 'spiderverse', 'newform', 'hermes', 'papers', 'discord', 'claude']);
  });

  it('gives each mode to exactly one potion', () => {
    const modes = SCHEMES.filter(s => s.mode).map(s => s.mode);
    expect([...modes].sort()).toEqual(['claude', 'discord', 'foudre', 'greek', 'hermes', 'newform', 'papers', 'slush', 'spiderverse']);
  ...
  it('never leaves two modes applied at once', () => {
    ...
    const ALL = ['sc-greek', 'sc-foudre', 'sc-slush', 'sc-spiderverse', 'sc-newform', 'sc-hermes', 'sc-papers', 'sc-discord', 'sc-claude'];
