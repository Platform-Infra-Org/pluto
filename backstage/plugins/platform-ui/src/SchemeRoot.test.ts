...
describe('colour schemes', () => {
  ...
  it('keeps one potion per design system, with stable ids', () => {
    ...
    expect(SCHEMES.map(s => s.id)).toEqual(['greek', 'foudre', 'slush', 'spiderverse', 'hanami', 'nightshade', 'newform', 'hermes', 'papers', 'discord', 'github', 'claude', 'dairy', 'obsidian']);
  });

  it('gives each mode to exactly one potion', () => {
    const modes = SCHEMES.filter(s => s.mode).map(s => s.mode);
    expect([...modes].sort()).toEqual(['claude', 'dairy', 'discord', 'foudre', 'github', 'greek', 'hanami', 'hermes', 'newform', 'nightshade', 'obsidian', 'papers', 'slush', 'spiderverse']);
  ...
  it('never leaves two modes applied at once', () => {
    ...
    const ALL = ['sc-greek', 'sc-foudre', 'sc-slush', 'sc-spiderverse', 'sc-hanami', 'sc-nightshade', 'sc-newform', 'sc-hermes', 'sc-papers', 'sc-discord', 'sc-github', 'sc-claude', 'sc-dairy', 'sc-obsidian'];
