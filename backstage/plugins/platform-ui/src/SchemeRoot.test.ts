...
describe('colour schemes', () => {
  ...
  it('keeps one potion per design system, with stable ids', () => {
    ...
    expect(SCHEMES.map(s => s.id)).toEqual(['greek', 'foudre', 'slush', 'spiderverse', 'hanami', 'nightshade', 'rimefast', 'egyptian', 'newform', 'hermes', 'papers', 'discord', 'github', 'claude', 'dairy', 'obsidian']);
  });
  ...
  it('never leaves two modes applied at once', () => {
    ...
    const ALL = ['sc-greek', 'sc-foudre', 'sc-slush', 'sc-spiderverse', 'sc-hanami', 'sc-nightshade', 'sc-rimefast', 'sc-egyptian', 'sc-newform', 'sc-hermes', 'sc-papers', 'sc-discord', 'sc-github', 'sc-claude', 'sc-dairy', 'sc-obsidian'];
